...
public class PersistentCookieStore implements CookieStore {
    ...
    protected void add(@NonNull HttpUrl uri, @NonNull Cookie cookie) {
        String name = getCookieToken(cookie);
//        LogUtils.i("CookieJar", "cookie name :" + cookie.name());
//        LogUtils.i("CookieJar", "cookie domain :" + cookie.domain());
//        LogUtils.i("CookieJar", "cookie expiresAt :" + cookie.expiresAt());
//        LogUtils.i("CookieJar", "cookie persistent :" + cookie.persistent());
//        LogUtils.i("CookieJar", "uri host:" + uri.host());

        /*long l = cookie.expiresAt();
        String expiresTime = DateUtils.formatDateByFormat(new Date(l), DateUtils.TIMESTAMP_PATTERN);
        LogUtils.i(TAG, "过期时间：" + expiresTime);*/
...
//        }
    ...
    @Override
    public void add(@NonNull HttpUrl uri, @NonNull List<Cookie> cookies) {
    ...
    @NonNull
    @Override
    public List<Cookie> get(@NonNull HttpUrl uri) {
        ArrayList<Cookie> ret = new ArrayList<Cookie>();
        if (cookies.containsKey(uri.host())) {
            Collection<Cookie> cookies = this.cookies.get(uri.host()).values();
            for (Cookie cookie : cookies) {
//                LogUtils.i(TAG, "cookie name :" + cookie.name());
//                LogUtils.i(TAG, "cookie domain :" + cookie.domain());
//                LogUtils.i(TAG, "cookie expiresAt :" + cookie.expiresAt());
//                LogUtils.i(TAG, "cookie persistent :" + cookie.persistent());
    ...
    @Override
    public boolean remove(@NonNull HttpUrl uri, @NonNull Cookie cookie) {
        String name = getCookieToken(cookie);
    ...
    @Nullable
    protected String encodeCookie(@Nullable SerializableHttpCookie cookie) {
        if (cookie == null)
            return null;
