...
import com.aide.financial.widget.dialog.config.MToastConfig;
...
public class MToast {
    ...
    private Toast make(MToastConfig config, @NonNull Context context, @NonNull CharSequence message, int duration) {
        ...
        myGrad.setCornerRadius(SizeUtils.dp2px(context, ToastBackgroundCornerRadius));
        myGrad.setColor(ToastBackgroundColor);
        myGrad.setStroke(SizeUtils.dp2px(context, ToastBackgroundStrokeWidth), ToastBackgroundStrokeColor);
        ...
        if (ToastGravity == MToastConfig.MToastGravity.CENTRE) {
            ...
            currentToast.setGravity(Gravity.BOTTOM, 0, SizeUtils.dp2px(context, 80));
        }
